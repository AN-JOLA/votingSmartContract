...
event Progress(address indexed sender, string indexed level );
    ...
    //function to convert name to bytes32 tye
    function convertName(string memory Iname) public pure returns (bytes32 name){
        bytes memory tempNameTest = bytes(Iname);
        if (tempNameTest.length == 0){
            return 0x0;
        }
        assembly{
            name := mload(add(Iname, 32)) 
        } 
    }
    ...
    function addCandidate(bytes32 name)  external onlyChairperson override returns (bool){
        ...
        candidates[name].name = convertName()
